...
- [x] **T28** Add a three-way view mode (Edit / Split / Preview) with side-by-side live preview (T28). *DoD: three modes cycled by Ctrl+E; defaultView persisted in localStorage with migration from defaultPreview; Split mode shows editor + live preview side-by-side with proportional scroll sync; hidden in preview; no data-model change (D4/D8).*
- [x] **T36** Add a fourth view mode, **Write** — a WYSIWYG contenteditable that round-trips to Markdown (the word-processor feel: edit the rendered document, markers hidden). *Verified (agent-browser, 2026-09-25): `js/ui/wysiwyg.js` is the second editor behind the swappable seam, so storage/search/export stay editor-agnostic (D8); marked→DOMPurify→contenteditable on mount and Turndown(+gfm)→Markdown on every settled edit, so `note.body` stays raw Markdown (D4); typing updates the body, toolbar Bold wraps a selection, switching to Edit shows the synced CodeMirror buffer, task-list checkbox toggle round-trips, Ctrl+Z undoes. Known v1 limits (documented in the file header): lossy round-trip for ambiguous syntax, caret drift when the caret sits inside consumed syntax, and own undo stack. No data-model change; `CACHE_VERSION` → noted-v16.*
- [x] **T37** Collapse the view modes to two — **Edit** (WYSIWYG) and **Code** (raw source) — and retire Split and Preview. *Once Edit renders as you type, Preview is just Edit with the cursor hidden and Split is two panes of the same document, so both were scaffolding. Removed the `.preview` pane, the `.editor-body.split` grid, the proportional scroll-sync helpers, and the now-dead `rendered` getter. `defaultView` migration is explicit via a `LEGACY_VIEW` table so no user's surface changes silently: old `edit` (raw source) → `code`; old `write`/`split`/`preview` (rendered) → `edit`. Fixed a one-directional buffer sync found while verifying: `setMode` pushed the WYSIWYG buffer out on leaving Edit but never pulled CodeMirror's in on entering, so Code-mode edits were invisible until you reloaded. Both hand-offs are now explicit.*
...
- [x] **T32** Add merge-based import for file-relay sync (per-id compare, live-wins ties, one transaction — D13). No server path: the blocked list stands. *Verified (agent-browser, 2026-09-24): newer-replaces / older-kept / one-side-adds; equal-ts ties both ways (incoming-deleted keeps live local; incoming-live restores deleted local); attachments add-if-absent; v1 payloads merge; replace mode blind-restores.* Next: T33 (worker index).
- [x] **T38** Reference attachments from the body with an `attachment:<id>` scheme (D14), so a stored image is embeddable in a note instead of dead-ended in the Attach panel. *Verified (agent-browser, 2026-09-26): real 172KB PNG pasted → byte-exact Blob in the `attachments` store, `![gradient.png](attachment:<id>)` in the body, no `blob:` leak, rendered to a live blob URL; Edit↔Code round-trip keeps the reference; export inlines the full payload as a 235,152-char `data:` URI (exactly `ceil(n/3)*4`) with the reference gone. Two ordering bugs found by instrumenting rather than guessing: resolution was cached on the whole body (every keystroke invalidated it, so each settle repainted and threw the caret) and is now cached on the reference set; and the note pane is behind an `x-if`, so `x-init` mounts the surface after `openNote` and its `setBody` guard never fired.*
- [x] **T39** Open a `.md`/`.markdown`/`.txt` file as a new note, and relabel the per-note Markdown download as Save. *Verified (agent-browser, 2026-09-26): `.md`, `.txt`, `.markdown` and extension-less files each create a note titled from the filename with a byte-exact body, persisted under a fresh UUIDv4, opening straight into the editor; `accept` carries the extensions ahead of the MIME types because `text/markdown` is not registered everywhere. One file = one note, so `multiple` stays off.*
...
- **v1** — T20 (CodeMirror 6 editor), T21 (trash view), T22 (keyboard shortcuts), T23 (settings), T24 (folders, v1→v2 migration), T25 (offline indicator + storage estimate), T26 (pinning, sort options, bulk Markdown zip — Dexie v3). **v1 complete.** P2: T30 done (vault encryption, D11), T31 (attachments, D12), T32 (merge sync, D13), T38 (attachment references, D14), T39 (open .md/.txt). Next: T33 (worker index, once notes exceed ~2,000).
